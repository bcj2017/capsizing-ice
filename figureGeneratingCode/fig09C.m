...
for j = 1:length(ts_arr) % iterate over chosen frames
    ...
    ft = abs(fft(curvature)); ft = ft.^2 / length(x_interp);
    pl = plot(1:1:ylimit-1,ft(2:ylimit));  
    xticks([0:5:ylimit]);
    pl.LineWidth = lw; pl.Color = cmap(clr_idx,:);
    if j == 2
        hYLabel2 = ylabel('Fourier amplitude, $|a_k|^2/N$','Interpreter','latex', 'Position', [-0.1, 0.5, 0],'FontSize',fontsize);
